...
check_parameters_exist($response_json, ["token", "email", "task_id"]);
...
try {
    ...
    if (!$db->assignTask($user["user_id"], $task["task_id"])) {
        $response_json["status"] = "Failed to assign ".$_GET["email"]." to task ".$task["task_id"];
...
$response_json["status"] = $_GET["email"]." has been assigned to task ".$task["task_id"].".";
